            ...
            <Col span="6" class="padding-left-10">
                <Card>
                    ...
                    <Row class="margin-top-20 publish-button-con">
                        ...
                        <span class="publish-button"><Button @click="handleLoadCache" type="dashed">读取缓存</Button></span>
                    </Row>
...
export default {
    ...
    methods: {
        ...
        handleLoadCache () {
            var tempBlog = localStorage.getItem("tempBlog_" + this.blog.id);
            if(tempBlog != undefined && tempBlog.length != 0 && tempBlog != JSON.stringify(this.blog)) {
                let temp = JSON.parse(tempBlog);
                this.$Modal.confirm({
                    title: '是否要放弃更改, 并使用本地缓存?',
                    content: '<h2>标题为《' + temp.blogTitle + '》</h2><h2>路径为"' + temp.displayUrl + '"</h2>',
                    okText: '使用',
                    cancelText: '不使用',
                    onOk: () => {
